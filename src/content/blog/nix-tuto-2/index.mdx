...
This is the primitive that Nix uses to define derivations. Packages are
derivations. But a derivation is something that is built into `/nix/store`. So
simpler things, that are not packages, can be derivations.
...
> [!IMPORTANT]
> - A derivation runs the `builder` with some `args`.
> - The environment variable $out is where the package results must be placed.
...
> [!IMPORTANT]
> `mkDerivation` has a number of implicit packages that are always available. It
> includes:
> - GCC
> - Bash
...
- `runCommand`, for when all you need is to run a command:
  ```nix
...
- `writeText`, that puts a string into $out
  ...
  ```
- Etc. Please take a look at the nixpkgs manual if you are curious about the
rest.
...
```console
$ nix build --impure --expr '((import <nixpkgs> {}).pkgs.formats.yaml {}).generate "config.yaml" { foo.bar = "baz"; }'
...
Nixpkgs also provides writers for scripting languages. The most simple one is
`writeShellScriptBin`, which can be used to quickly create a new usable package.
It is similar to `writeText`, but also placing the file in `$out/bin` and making
it executable:

```console
$ nix build --impure --expr '(import <nixpkgs> {}).writeShellScriptBin "test" "echo test"'
...
function. This function, will accept some extra attributes, that mean something
to the builder.

Some other languages, like python, have an scope of packages. For example, to
acess the different python libraries, you use `pkgs.python3.pkgs.<name>`. If
that is the case, then it will also define a custom `callPackage`:
